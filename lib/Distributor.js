...
module.exports = class Distributor {
     ...
     * @returns {Promise}
     */
    distributeFile(file, targetFile, leaveConnectionOpen) {
        var filePath = null;
        ...
        return Promise.map(this.servers, (server) => {
            ...
            return server.transfer(filePath, targetFile, leaveConnectionOpen);
        }).then(() => {
            if (this.config.debug) {
        ...
        return this.connectAllServers()
            .then(() => {
                return Promise.map(files, (file) => {
                    return this.distributeFile(file, targetFiles[files.indexOf(file)], true);
                }, {
                ...
                return this.disconnectAllServers();
            }, (err) => {
                if (this.config.debug) {
                    console.error(err);
                }
                return this.disconnectAllServers().then(() => {
                    return Promise.reject(err);
                });
            });
    }
