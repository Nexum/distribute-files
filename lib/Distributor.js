...
module.exports = class Distributor {
     ...
     *  {
     *      debug: Boolean,
     *      root: String,    // The Root for all files, leave empty if none should be used, you will have to supply an absolute path later
     *      servers: [
     *          {
     *              type: "ftp",
    ...
    distributeFile(file) {
        return new Promise((resolve, reject) => {
            ...
            try {
                ...
                console.log(e);
                return reject(new Error("File " + filePath + " does not exists"));
            }
            ...
            return Promise.map(this.config.servers, (server) => {
                ...
                return transfers[server.type](filePath);
            });
        })
    }
     ...
     * @returns {Promise}
     */
    distibuteFiles(files) {
        return Promise.map(files, (file) => {
            return this.distibuteFile(file);
        });
    }
